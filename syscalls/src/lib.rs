...
mod tests {
    ...
    fn test_syscall_poseidon() {
        ...
        assert_eq!(
            poseidon(true, poseidon::Endianness::LittleEndian, &[1; 32]).unwrap(),
            0 /* success */
        );
        assert_eq!(
            poseidon(true, poseidon::Endianness::BigEndian, &[1; 32]).unwrap(),
            0 /* success */
        );
        assert_eq!(
            poseidon(true, poseidon::Endianness::LittleEndian, &[1; 31]).unwrap(),
            1 /* error */
        );
        assert_eq!(
            poseidon(true, poseidon::Endianness::BigEndian, &[1; 31]).unwrap(),
            1 /* error */
        );

        // Without enforced padding.
        assert_eq!(
            poseidon(false, poseidon::Endianness::LittleEndian, &[1; 32]).unwrap(),
            0 /* success */
        );
        assert_eq!(
            poseidon(false, poseidon::Endianness::BigEndian, &[1; 32]).unwrap(),
            0 /* success */
        );
        assert_eq!(
            poseidon(false, poseidon::Endianness::LittleEndian, &[1; 31]).unwrap(),
            0 /* success */
        );
        assert_eq!(
            poseidon(false, poseidon::Endianness::BigEndian, &[1; 31]).unwrap(),
            0 /* success */
        );
    }
