...
impl PacketRead for BytesPacket {
    fn data<I>(&self, index: I) -> Option<&<I as SliceIndex<[u8]>>::Output>
    where
        I: SliceIndex<[u8]>,
    {
...
impl<'a> PacketRead for PacketRef<'a> {
    fn data<I>(&self, index: I) -> Option<&<I as SliceIndex<[u8]>>::Output>
    where
        I: SliceIndex<[u8]>,
    {
...
impl<'a> PacketRead for PacketRefMut<'a> {
    fn data<I>(&self, index: I) -> Option<&<I as SliceIndex<[u8]>>::Output>
    where
        I: SliceIndex<[u8]>,
    {
...
impl<'a> IndexedParallelIterator for PacketBatchParIter<'a> {
    fn len(&self) -> usize {
        match self {
...
impl<'a> IndexedParallelIterator for PacketBatchParIterMut<'a> {
    fn len(&self) -> usize {
        match self {
            Self::Pinned(iter) => iter.len(),
            Self::Bytes(iter) => iter.len(),
...
impl Index<usize> for PinnedPacketBatch {
    type Output = Packet;
...
impl IndexMut<usize> for PinnedPacketBatch {
    #[inline]
