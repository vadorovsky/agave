...
impl PacketRefMut<'_> {
    ...
    #[inline]
    pub fn copy_from_packet(&mut self, src_packet: &PacketRef<'_>) {
        *self.meta_mut() = src_packet.meta().clone();
        match self {
            Self::Packet(self_packet) => {
                if let Some(data) = src_packet.data(..) {
                    let size = src_packet.size();
                    self_packet.buffer_mut()[..size].copy_from_slice(data);
                }
            }
            Self::Bytes(self_packet) => {
                self_packet.meta_mut().size = usize::MAX;
                self_packet.buffer = match src_packet.data(..) {
        ...
        T: solana_packet::Encode,
    {
        ...
        for (addr, data) in dests_and_data {
            ...
            if !addr.ip().is_unspecified() && addr.port() != 0 {
                let packet = match Packet::from_data(Some(addr), data) {
                    Ok(packet) => packet,
                    Err(e) => {
...
impl FromIterator<Packet> for PinnedPacketBatch {
...
pub fn to_bytes_packet_batches<T: Serialize>(
    items: &[T],
    chunk_size: usize,
...
impl BytesPacketBatch {
    ...
    pub fn to_pinned_packet_batch(&self) -> PinnedPacketBatch {
        let mut batch = PinnedPacketBatch::with_capacity(self.len());
        for bytes_packet in self.iter() {
            let mut packet = Packet::default();
            let size = bytes_packet.size();
