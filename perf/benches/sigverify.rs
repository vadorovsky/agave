...
use {
    ...
    rand::{thread_rng, Rng},
    solana_perf::{
        packet::{to_packet_batches, PacketRead, TpuPacket, TpuPacketBatch},
        recycler::Recycler,
        sigverify,
...
fn bench_sigverify_uneven(bencher: &mut Bencher) {
    ...
    while current_packets < num_packets {
        ...
        let batch = (0..len)
            .map(|_| {
                let tx = if thread_rng().gen_ratio(1, 2) {
                    simple_tx.clone()
                } else {
                    multi_tx.clone()
                };
                let mut packet = TpuPacket::from_data(None, &tx).expect("serialize request");
