...
impl ReadOnlyAccountsCache {
    ...
    fn evict(
        ...
        while data_size.load(Ordering::Relaxed) > target_data_size {
            // 1st method - pick single elements from the cache, free the
            // read lock immediately after picking each of them.
            // Do it manually, without min_by_keys or other iterator
            // primitives (apart from choosing the single sample, where that's
            // necessary).
            // Perform the search for minimum element without locking.
            //
            // let key = {
            //     let mut min = u32::MAX;
            //     let mut to_evict = None;
            //     for _ in 0..evict_sample_size {
            //         let (key, last_update_time) = cache
            //             .iter()
            //             .choose(&mut rng)
            //             .map(|entry| {
            //                 (
            //                     entry.key().to_owned(),
            //                     entry.value().last_update_time.load(Ordering::Relaxed),
            //                 )
            //             })
            //             // PANICS: Impossible to happen for a non-empty batch of entries.
            //             .unwrap();
            //         if last_update_time >= min {
            ...
            // 2nd method - consume the cache as an iterator, retrieve the
            // element as owned during mapping, pick K elements with
            // choose_multiple.
            // Perform the search for minimum element without locking.
            //
            // let key = {
            //     let entries = cache
            //         .iter()
            //         // DashMap iterator yields a RefMulti, which holds a read
            //         // lock on the map. We obtain owned values to free that
            //         // lock.
            //         // Doing so means that the lines below, searching for the
            //         // minimum element in the sample, are not the part of
            //         // critical section. That might result in a race condition,
            //         // where our picked key might be updated. However, we are
            //         // willing to accept that in exchange for smaller lock
            //         // contention. Keeping the lock for the whole closure
            //         // results in 1-2% worse performance.
            //         .map(|entry| {
            //             (
            //                 entry.key().to_owned(),
            //                 entry.value().last_update_time.load(Ordering::Relaxed),
            //             )
            //         })
            //         .choose_multiple(&mut rng, evict_sample_size);
            //     let (key, _) = entries
            //         .iter()
            //         .min_by_key(|(_, last_update_time)| last_update_time)
            //         // PANICS: Impossible to happen for a non-empty batch of entries.
            //         .unwrap();
            //     key.to_owned()
            // };

            // 3rd method - consume the cache as an iterator, hold the RefMultis
            // yielded by it till the end of closure, so the search for the
            // minimum element happens with the read lock acquired.
            // Version of the code which keeps the read lock also when picking
            // up the minimum element. It performs 1-2%  worse in benchmarks:
            //
            let key = {
                let entries = cache.iter().choose_multiple(&mut rng, evict_sample_size);
                let entry = entries
                    .iter()
                    .min_by_key(|entry| entry.value().last_update_time.load(Ordering::Relaxed))
                    // PANICS: Impossible to happen for a non-empty batch of entries.
                    .unwrap();
                // key() returns a RefMulti, which holds a read lock on the
                // DashMap. Obtaining an owned key drops the RefMulti and
                // releases the lock.
                entry.key().to_owned()
            };
...
mod tests {
    ...
    fn test_read_only_accounts_cache_deterministic(evict_sample_size: usize) {
        solana_logger::setup();
