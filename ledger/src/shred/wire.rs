...
#[inline]
pub fn get_shred<'a>(packet: &'a PacketRef) -> Option<&'a [u8]> {
...
#[inline]
pub fn get_shred_and_repair_nonce<'a>(packet: &'a PacketRef) -> Option<(&'a [u8], Option<Nonce>)> {
    let data = packet.data(..)?;
...
mod tests {
    ...
    fn test_merkle_shred_wire_layout(repaired: bool, chained: bool, is_last_in_slot: bool) {
        ...
        for shred in &shreds {
            ...
            let bytes = get_shred(&packet_ref).unwrap();
