...
pub fn verify_shreds_gpu(
    ...
    elems.extend(batches.iter().map(|batch| {
...
mod tests {
    ...
    fn run_test_sigverify_shred_cpu(slot: Slot) {
        ...
        // packet.buffer_ mut()[..shred.payload().len()].copy_from_slice(shred.payload());
    ...
    fn make_packets<R: Rng>(rng: &mut R, shreds: &[Shred]) -> Vec<PacketBatch> {
        let packets = shreds
            .iter()
            .map(|shred| shred.to_packet())
            .collect::<BytesPacketBatch>();
        let packets: Vec<_> = repeat_with(|| {
            let size = rng.gen_range(0..16);
            let batch = PacketBatch::from(packets.clone());
