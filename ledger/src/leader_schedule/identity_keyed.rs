...
impl LeaderSchedule {
    ...
    pub fn new_from_schedule_with_repeat(slot_leaders: Vec<Pubkey>, len: u64, repeat: u64) -> Self {
        Self {
            leader_slots_map: Self::invert_slot_leaders(&slot_leaders),
            slot_leaders,
            len,
    ...
    pub fn get_slot_leaders(&self) -> impl Iterator<Item = &Pubkey> {
        self.slot_leaders
            .iter()
            .flat_map(|leader| iter::repeat_n(leader, self.repeat as usize))
    }
...
impl Index<u64> for LeaderSchedule {
    ...
    fn index(&self, index: u64) -> &Pubkey {
        let chunk_index = (index / self.repeat) as usize % self.slot_leaders.len();
        &self.slot_leaders[chunk_index]
    }
}
