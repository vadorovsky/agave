use {
    ...
    crate::{
        bank::{
            partitioned_epoch_rewards::PartitionedStakeRewards, PrevEpochInflationRewards,
            RewardCalcTracer, RewardCalculationEvent, RewardsMetrics, VoteReward, VoteRewards,
        },
        inflation_rewards::{
...
impl Bank {
    ...
    fn get_epoch_reward_calculate_param_info<'a>(
        ...
        let (stake_delegations, stake_delegations_by_voter) = self.filter_stake_delegations(stakes);
    ...
    fn calculate_stake_vote_rewards(
        ...
        let thread_pool =
            crate::thread_pool::ThreadPool::new("solBnkStkVoteRwrds", num_cpus::get() / 4);
        let (vote_account_rewards_tx, vote_account_rewards_rx) = mpsc::channel();
        let (stake_rewards_tx, stake_rewards_rx) = mpsc::channel();

        let reward_calc_tracer = Arc::new(reward_calc_tracer);

        let (_, measure_stake_rewards_us) = measure_us!(thread_pool.scope(|s| {
            for (voter_pubkey, stake_accounts) in stake_delegations_by_voter {
                let vote_account_rewards_tx = vote_account_rewards_tx.clone();
                let stake_rewards_tx = stake_rewards_tx.clone();
                ...
                s.spawn(move || {
                    let vote_account_from_cache = cached_vote_accounts.get(voter_pubkey);

                    if ASSERT_STAKE_CACHE && vote_account_from_cache.is_none() {
                        let account_from_db = self.get_account_with_fixed_root(voter_pubkey);
                        if let Some(account_from_db) = account_from_db {
                            ...
                            ) && VoteAccount::try_from(account_from_db.clone()).is_ok()
                            {
                                panic!(
                                    ...
                                    voter_pubkey, account_from_db
                                );
                            }
                        }
                    }
                    let vote_account =
                        vote_account_from_cache.expect("could not find the vote account in cache");
                    let vote_state_view = vote_account.vote_state_view();
                    let commission = vote_state_view.commission();
                    let mut voters_reward_entry = VoteReward {
                        commission,
                    ...
                    vote_account_rewards_tx
                        .send((**voter_pubkey, voters_reward_entry))
                        .unwrap();
                });
            }
        }));
        drop(vote_account_rewards_tx);
        drop(stake_rewards_tx);
        let vote_account_rewards: VoteRewards = vote_account_rewards_rx.iter().collect();
        let stake_rewards: PartitionedStakeRewards = stake_rewards_rx.iter().collect();
